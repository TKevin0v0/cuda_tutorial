...
const int N = 1024 * 1024; //要处理的数据量

//cpu实现
float reduce_cpu(const std::vector<float> &h_data)
{
    float sum = 0.f;
    for(int i = 0; i < N; i ++)
        sum += h_data[i];
...
int main()
{
    ...
    /*
    每个block处理1024个数据，最后再把每个block的结果加起来
    */
    ...
    // GPU 计时结束
    td::cout << "GPU kernel time: " << milliseconds << " ms" << std::endl;
