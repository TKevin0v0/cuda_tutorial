...
__global__ void mysgemm_v1(int M, int N, int K, float alpha, float *A, float *B,
                            float beta, float *C)
{
    int gx = blockIdx.x * blockDim.x + threadIdx.x; //x是列
    int gy = blockIdx.y * blockDim.y + threadIdx.y; //y是行
    if(gx >= N || gy >= M) return;
    float tmp = 0.f;
    for(int i = 0; i < K; i ++)
    ...
    for(int N : sizes)
    {
        ...
        try
        {
            ...
            checkCudaError(cudaEventSynchronize(stop),
                        "cudaEventSynchronize cublas failed"); //CPU等待，直到GPU执行到stop event为止。
            float cublas_time = 0;
            ...
            for(int i = 0; i < warpup_time; i ++) //热身
        ...
        else csv_file << N << ",OOM,OOM,0" << std::endl;
    }
    csv_file.close();
