...
/*
v2：利用共享显存，把c结果矩阵分成多个tile，一个block处理一个tile，c的结果就是把每个tile的部分和
全部加起来
*/
                            ...
                            float beta, float *C)
{
  ...
  const int BK = BLOCK_SIZE; //A的列或者B的行，每次切多少
  //当前线程在 C tile 中的坐标，这个线程负责算：C[ty][tx]
  int tx = threadIdx.x % BN;
  int ty = threadIdx.x / BN;
  __shared__ float As[BM * BK];
  __shared__ float Bs[BK * BN];
  /*
  A 偏移到当前 block 对应的 A 行块起始位置
  B 偏移到当前 block 对应的 B 列块起始位置
  C 偏移到当前 block 对应的 C tile 左上角
  */
  A = &A[by * BM * K];
  B = &B[bx * BN];
  C = &C[by * BM * N + bx * BN];
...
#define CEIL_DIV(M, N) ((M) + (N) - 1) / (N) // M/N向上取整
int main() 
{
