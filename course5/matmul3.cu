                           ...
                           float beta, float *C)
{
    ...
    /*
    上个优化中每个线程每次搬运一个数据，因此总共需要执行 a_tile_stride 移动的步长。而本节
    课采用向量化加载指令，单次可搬运多个数据，从而减少了总的移动次数。
    */
    const int ldg_a_num = BM * BK / thread_num / 4; // 每个线程需要执行多少次float4加载
    const int ldg_b_num = BK * BN / thread_num / 4;
    int a_tile_row = threadIdx.x / (BK / 4);
    /*
    threadIdx.x % (BK/4)表示当前线程负责哪一个float4列块,再*4表示转回真实float列坐标
    */
    int a_tile_col = threadIdx.x % (BK / 4) * 4;
    int a_tile_stride = BM / ldg_a_num; // 当前线程下一轮加载时跨越多少行

    int b_tile_row = threadIdx.x / (BN / 4);
    int b_tile_col = threadIdx.x % (BN / 4) * 4;
    int b_tile_stride = BK / ldg_b_num;

    float accum[TM][TN] = {0.f}; // 当前线程负责计算的TM×TN结果块,过程中一直累加到最终的结果
    /*
    因为FETCH_FLOAT4存储要求目标地址连续，而A放入AS需要转置，地址不连续，所以需要ldg_a_reg来过渡一下。
    */
    float ldg_a_reg[4 * ldg_a_num] = {0.f}; // 4*ldg_a_num就是当前线程总共需要加载多少个float
    float a_frag[TM];
    float b_frag[TN];
    A = &A[by * BM * K];
    B = &B[bx * BN];
    ...
    for (int k = 0; k < K; k += BK)
    {
        ...
        {
            // 这里*4是因为因为一次FETCH_FLOAT4会处理4个float，所以寄存器索引也必须按4个float递增
            int ldg_index = i / a_tile_stride * 4;
            FETCH_FLOAT4(ldg_a_reg[ldg_index]) = FETCH_FLOAT4(A[OFFSET(a_tile_row + i, a_tile_col, K)]);
            for (int j = 0; j < 4; j++)
                As[OFFSET(a_tile_col + j, i + a_tile_row, BM)] = ldg_a_reg[ldg_index + j];
        }
        ...
        for (int i = 0; i < BK; i++)
        {
            // 把shared memory的数据提前缓存到寄存器，这样后面的访问运算就会变得更快
