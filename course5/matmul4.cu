               ...
               float *C)
{
    ...
    for (int i = 0; i < BM; i += a_tile_stride)
    {
        ...
        for (int j = 0; j < 4; j++)
            As[0][OFFSET(a_tile_col + j, i + a_tile_row, BK)] = ldg_a_reg[ldg_index + j];
    }
    ...
    do
    {
        ...
        if (k < K)
        {
            for (int i = 0; i < BM; i += a_tile_stride)
            {
                ...
                FETCH_FLOAT4(ldg_a_reg[ldg_idx]) = FETCH_FLOAT4(A[OFFSET(a_tile_row + i, a_tile_col + k, K)]);
            }
            for (int i = 0; i < BK; i += b_tile_stride)
            {
                int ldg_idx = i / b_tile_stride * 4;
                FETCH_FLOAT4(ldg_b_reg[ldg_idx]) = FETCH_FLOAT4(B[OFFSET(k + b_tile_row + i, b_tile_col, N)]);
            }
        }
        load_idx = write_idx ^ 1;
        // 注意，这里还有最后一个循环bk = BK-1没计算，要在最后计算
        for (int bk = 0; bk < BK - 1; bk++)
        {
            for (int m = 0; m < TM; m += 4)
                FETCH_FLOAT4(a_frag[(bk + 1) % 2][m]) =
                    FETCH_FLOAT4(As[load_idx][OFFSET(bk + 1, ty + m, BM)]);
            for (int n = 0; n < TN; n += 4)
                FETCH_FLOAT4(b_frag[(bk + 1) % 2][n]) =
                    FETCH_FLOAT4(Bs[load_idx][OFFSET(bk + 1, tx + n, BN)]);
            for (int m = 0; m < TM; m++)
                for (int n = 0; n < TN; n++)
        ...
        if (k < K)
        {
            ...
            {
                int ldg_idx = i / a_tile_stride * 4;
                // 这里不是连续地址，所以不能用FETCH_FLOAT4
                for (int j = 0; j < 4; j++)
                    As[write_idx][OFFSET(a_tile_col + j, i + a_tile_row, BM)] =
                        ldg_a_reg[ldg_idx + j];
            }
            for (int i = 0; i < BK; i += b_tile_stride)
            {
                int ldg_idx = i / b_tile_stride * 4;
                FETCH_FLOAT4(Bs[write_idx][OFFSET(b_tile_row, b_tile_col, BN)]) =
                    FETCH_FLOAT4(ldg_b_reg[ldg_idx]);
            }
            __syncthreads();
            for (int m = 0; m < TM; m++)
                FETCH_FLOAT4(a_frag[0][m]) =
                    FETCH_FLOAT4(As[write_idx][OFFSET(0, ty + m, BM)]);
            for (int n = 0; n < TN; n++)
                FETCH_FLOAT4(b_frag[0][n]) =
                    FETCH_FLOAT4(Bs[write_idx][OFFSET(0, tx + n, BN)]);
            write_idx ^= 1;
        }
        for (int m = 0; m < TM; m++)
            for (int n = 0; n < TN; n++)
