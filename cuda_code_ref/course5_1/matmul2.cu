...
/*
BM：一个 block 负责c tile的行
BN：一个 block 负责c tile的列
BK：K 方向每次分块多长（分段计算用）
TM：一个线程在 M 方向计算多少行
TN：一个线程在 N 方向计算多少列
M, N, K：矩阵 A (M,K) × B (K,N) = C (M,N)
A, B, C：全局内存里的矩阵
*/
...
__global__ void mysgemm_v4(int M, int N, int K, float alpha, float *A, float *B,
                           float beta, float *C)
{
  int bx = blockIdx.x;                                  // 当前 block 在 grid 里是第几列块
  int by = blockIdx.y;                                  // 当前 block 在 grid 里是第几行块
  int block_row_thread = BN / TN;                       // 一个tile的一行，需要多少个线程才能覆盖所有列
  int block_col_thread = BM / TM;                       // 一个tile的一列，需要多少个线程才能覆盖所有行
  int thread_num = block_row_thread * block_col_thread; // 一个block里需要处理数据的总线程数
  int tx = (threadIdx.x % block_row_thread) * TN;       // 线程负责的 N 方向起始列（tx = 横向第几号线程 × TN）
  int ty = (threadIdx.x / block_row_thread) * TM;       // 线程负责的 M 方向起始行（ty = 纵向第几号线程 × TM）
  __shared__ float As[BM * BK];
  __shared__ float Bs[BK * BN];
  A = &A[by * BM * K];
  B = &B[bx * BN];
  C = &C[by * BM * N + bx * BN];       // 当前线程负责计算 C 的起始行和起始列
  int a_tile_row = threadIdx.x / BK;   // 当前线程加载 A 的起始行
  int a_tile_col = threadIdx.x % BK;   // 当前线程加载 A 的起始列
  int a_tile_stride = thread_num / BK; // 线程下一次搬运时，跳多少行执行下一次搬运
  ...
  for (int k = 0; k < K; k += BK)
  {
    ...
    for (int i = 0; i < BK; i += b_tile_stride)
      Bs[(b_tile_row + i) * BN + b_tile_col] = B[(b_tile _row + i) * N + b_tile_col];
    __syncthreads();
    A += BK;
    B += BK * N;
    for (int i = 0; i < BK; i++)
    {
      ...
      {
        for (int l = 0; l < TN; l++)
          tmp[j][l] += As[(ty + j) * BK + i] * Bs[tx + l + i * BN]; // AS: 取 A 共享内存里的第 (ty+j) 行、第 i 列
                                                                    // BS: 取 B 共享内存里的第 i 行、第 (tx+l) 列
      }
    }
    __syncthreads();
  }
