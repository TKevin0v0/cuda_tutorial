                           ...
                           float beta, float *C)
{
  ...
  const int ldg_a_num = BK * BM / thread_num / 4; // 每个线程需要加载的轮数
  const int ldg_b_num = BK * BN / thread_num / 4; //

  int a_tile_row = threadIdx.x / (BK / 4);
  int a_tile_col = threadIdx.x % (BK / 4) * 4;
  int a_tile_stride = BM / ldg_a_num; // 线程加载完一轮需要跨越的行数
  ...
  float accum[TM][TN] = {0.};
  /*
  线程从global memory加载来的float4临时缓存,之后要放到shared memory中
  需要的原因：FETCH_FLOAT4(...)得到的是float4，是4个float组成的向量。但是shared memory这里的As[]并不是float4数组
  ，而是float As[]，也就是普通的float数组
  */
  ...
  for (int k = 0; k < K; k += BK)
  {
    ...
    {
      int ldg_index = i / a_tile_stride * 4; // 当前这轮float4应该写入寄存器数组的哪个起始位置
      // 一次性读4个float，存到寄存器数组ldg_a_reg里
      FETCH_FLOAT4(ldg_a_reg[ldg_index]) = FETCH_FLOAT4(A[OFFSET(a_tile_row + i, a_tile_col, K)]);
      // 把4个元素分别存到共享内存中，这里是转置存储（防止bank冲突）
