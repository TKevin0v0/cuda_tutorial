               ...
               float *C)
{
  ...
  __syncthreads();
  // 因为在计算bk内的数据也要用双缓冲。所以提前准备第0个bk的数据
  ...
  int write_index = 1; // 表示下一轮数据写到哪个 shared memory buffer
  int load_index;
  int k = 0;
  do
  {
    // 提前把下一轮 BK tile从 global memory的A / B加载到ldg_a_reg和ldg_b_reg
    k += BK;
    if (k < K)
    {
      for (int i = 0; i < BM; i += a_tile_stride)
    ...
    load_index = write_index ^ 1; // load_index是另外一组shared memory buffer
    // 在计算bk的时候，也用了双缓冲的思想
    // 注意，这里还有最后一个循环bk = BK-1没计算，要在最后计算
    for (int bk = 0; bk < BK - 1; bk++)
    {
      // 在计算bk这一层的时候，提前把bk+1的数据读到另一组 register buffer。
      for (int m = 0; m < TM; m += 4)
    ...
    // 这里是把for (int bk = 0; bk < BK - 1; bk++)这里没算完的BK-1算完
    for (int m = 0; m < TM; m++)
      for (int n = 0; n < TN; n++)
...
int main()
{
  ...
  // 打开CSV文件
  std::ofstream csv_file("sgemm_benchmark_double_buffering.csv");
