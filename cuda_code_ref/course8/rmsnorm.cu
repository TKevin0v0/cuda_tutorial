...
__inline__ __device__ float block_reduce(float val)
{
  ...
  for (int offset = warpSize / 2; offset > 0; offset /= 2)
    val += __shfl_down_sync(0xFFFFFFFF, val, offset); // warp里所有活跃线程同时执行这条指令
...
// 一个线程一次读取四个数据，利用float4类型进行内存访问优化
                                         ...
                                         int batch, int size, float eps)
{
  ...
  float *block_in = in + bid * size;   // 当前block要处理数据的起始位置
  float *block_out = out + bid * size; // 当前block输出数据的起始位置
  constexpr int pack_size = 4;
  const int pack_num = size / pack_size;     // 能被float4覆盖的元素数量
  const int pack_off = pack_size * pack_num; // 已经被float4覆盖的元素数量，剩下如果有几个元素没有被float4覆盖，就由下面的标量代码处理。

  float sum = 0.0f;
  /*
  从[1,2,3,4,5,6,7,8]到float4[0] = [1,2,3,4]，float4[1] = [5,6,7,8]这种内存访问方式
  */
  ...
  float4 *wei_pack = reinterpret_cast<float4 *>(wei);       // 权重的类型变化了
  float4 *out_pack = reinterpret_cast<float4 *>(block_out); // 输出也按float4写
  for (int i = tid; i < pack_num; i += blockDim.x)
  {
    float4 in_float4 = *(in_pack + i);
...
/*
1.返回的是block 的 thread0 中的sum，也就是block里面所有数据的和。其他线程返回的都是0
2.必须同一个 warp 内所有“活跃线程”一起执行__shfl_down_sync
*/
...
int main()
{
  const int batch = 16;    // 16个样本
  const int size = 1024;   // 每个样本1024维特征
  const float eps = 1e-6f; // 公式里面分母的eps，防止除0错误
  const int total = batch * size;

  std::vector<float> h_input(total);       // 输入矩阵
  std::vector<float> h_weight(size);       // 每个特征的权重
  std::vector<float> h_output_cpu(total);  // cpu结果
  std::vector<float> h_output_cuda(total); // gpu结果

  std::random_device rd;
  std::mt19937 gen(rd());
  std::normal_distribution<float> dis(0.0f, 1.0f); // 正态分布随机数生成器
  // 初始化输入
  ...
  const int grid_size = batch; // 一个block处理一个batch
  dim3 grid(grid_size);
