...
{
  int groupSeq = (seqlen + Bc - 1) / Bc; // 表示整个 K/V 被切成多少部分。每个部分有 Bc 行
  ...
  __shared__ FP sO[Br][dim];    // 当前 block 内每个 Query 输出向量的 累积分子
  __shared__ FP sQK[Br][Bc];    // sQK[Br][Bc] 里面的每一个元素，都是一个完整的 Q·K 点积结果，不是部分结果
  __shared__ FP sSafeE[Br][Bc]; // exp(score-max)的结果
  __shared__ FP sDenom[Br];     // 分母累计值
  __shared__ FP sMax[Br];       // 之前每一行中所有 KV Tile 的最大值
  int tx = threadIdx.x;
  int ty = threadIdx.y;
  int row = ty + blockIdx.y * blockDim.y; // 当前线程处理的是哪一个query
  if (row >= seqlen)
    return; // 防止多开的一个block中的线程越界
  // 在加载 query 向量，需要groupTx次才能搬运完成。这里就是分割了每个Q的dim，分批次运到共享内存当中
  for (int i = 0; i < groupTx; i++)
  {
    sQ[ty][i * blockDim.x + tx] = Q[row * dim + i * blockDim.x + tx]; // 把Q从global memory搬到shared memory。
    sO[ty][i * blockDim.x + tx] = 0;
  }
  sMax[ty] = -INFINITY; // 之前所有tile的最大值
  sDenom[ty] = 0;       // 当前分母
  for (int j = 0; j < groupSeq; j++)
  {
    if ((j * Bc + tx) < seqlen)         // 防止当前的线程越界，就是处理超过了seqlen的数据的部分，这部分数据是没有的。
      for (int i = 0; i < groupTy; i++) //  要分多少次把 K/V 的 dim 维度搬进共享内存
      {
        // tx 对应 KV Tile 的行，ty 对应 dim 的一部分
    ...
    __syncthreads();
    // localMax:sQK矩阵中每一行最大值
    FP localMax = -INFINITY;
    for (int i = 0; i < Bc; i++)
      localMax = max(localMax, sQK[ty][i]);
    __syncthreads();
    FP newMax = max(sMax[ty], localMax); // newMax就是当前sQK矩阵的最大值和以往最大值比较得到的最大值
    sSafeE[ty][tx] = exp(sQK[ty][tx] - newMax);
    __syncthreads();
    FP localDenom = 0.f; // localDenom:softmax 分母
    for (int i = 0; i < Bc; i++)
      localDenom += sSafeE[ty][i];
    __syncthreads();
    FP rescaleOld = exp(sMax[ty] - newMax);
    FP newDenom = sDenom[ty] * rescaleOld + localDenom; // newDenom：更新最大值后的softmx的分母
    // 这里以sO的角度去思考
    for (int i = 0; i < groupTx; i++) // 遍历输出向量的dim维
    {
      sO[ty][i * Bc + tx] = sO[ty][i * Bc + tx] * rescaleOld; // softmax的分子更新，sO 维护的是 softmax 分子
      /*
      这个循环对应的是有道笔记flashattention原理举例子的这里：[0.42+0.32+0.48+1.04，4.2+3.2+4.8+10.4，42+32+48+104]。
      只是分子要不断相加，所以是+=
      */
      for (int k = 0; k < Bc; k++)
        sO[ty][i * Bc + tx] += sSafeE[ty][k] * sV[k][i * Bc + tx]; // 乘v的这里是加权求和，不是向量点积！！！！
    }
